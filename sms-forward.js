...
!(async () => {
  ...
  const fn = async (key, index) => {
    ...
    const KEY_CUSTOM_POST = `@ChinaTelecomOperators.${key}.custom_post`
    ...
    await notify(title, subtitle, body, { copy, KEY_PUSHDEER, KEY_BARK, KEY_CUSTOM_POST })
    $.log(`👉🏻 [${index}][${key}] 配置结束`)
  }
...
async function notify(title, subtitle, body, { copy, KEY_PUSHDEER, KEY_BARK, KEY_CUSTOM_POST }) {
  const pushdeer = $.getdata(KEY_PUSHDEER)
  const bark = $.getdata(KEY_BARK)
  const customPost = $.getdata(KEY_CUSTOM_POST)

  if (pushdeer || bark || customPost) {
    if (pushdeer) {
      try {
    ...
    if (customPost) {
      try {
        const [url, postBody] = customPost.split('\n')
        const pushContent = `${subtitle}\n${body}`
        const pushFullText = `${title}\n${pushContent}`

        const finalUrl = url
          .replace(/\[推送标题]/g, encodeURIComponent(title))
          .replace(/\[推送内容]/g, encodeURIComponent(pushContent))
        ...
        const finalBody = postBody
          ...
          .replace(/\[推送内容]/g, pushContent)
          .replace(/\[复制内容]/g, copy)
          .replace(/\[推送全文]/g, pushFullText)
        ...
        $.log('↓ res body')
        console.log($.toStr(resBody))
         if (!['0', '200', 201].includes(String($.lodash_get(resBody, 'code'))) && !$.lodash_get(resBody, 'isSuccess') && status != 200) {
          throw new Error($.lodash_get(resBody, 'errorMessage') || $.lodash_get(resBody, 'message') || $.lodash_get(resBody, 'msg') || '未知错误')
        }
      } catch (e) {
        console.log(e)
        $.msg('短信转发', `❌ customPost 请求`, `${$.lodash_get(e, 'message') || $.lodash_get(e, 'error') || e}`, {})
      }
    }
  } else {
